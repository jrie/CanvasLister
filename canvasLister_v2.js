...
function canvasLister_v2(canvasItemId, sourceFile, fontDefaultFamily, fontDefaultSize, fontDefaultWeight, fontDefaultShape, textAlignment, backgroundColor, fontDefaultColor, sourceText) {
    ...
    function simpleParse(formatData) {
        ...
        if (imageData !== null) {
            ...
            for (var img = 0; img < imageData.length; img++) {
                ...
                formatData = formatData.replace(imageTag, imgTagAlias);
                
                // Clean img from imageTag
                imageTag = imageTag.replace('img', '');
                ...
                // Get keys and values for the image
                keyValues = imageTag.match(imgkeyValueMatch);
                
                // Try to filter the keyValue match from random data
                for (var keyValue = 0; keyValue < keyValues.length; keyValue++) {
                    if (keyValues[keyValue].indexOf(">") !== -1) {
                        keyValues = keyValues.slice(0, keyValue);
                        break;
                    }
                }
        ...
        for (var tag = 0; tag < tagLength; tag++) {
            ...
            if (currentTag === '<_img_>' && formatData.trim().indexOf(currentTag) === 0) {
                if (imgPointer < parserObject.imageStore.length) {                   
                    parserObject.data.push(currentTag);
                    parserObject.orders.push( parserObject.imageStore[imgPointer].id);
                    parserObject.dataPoints.push(dataPoint);
                    imgPointer++;
                    formatData = formatData.substr(currentTag.length);
                }
        ...
        for (var data = 0; data < parserObject.data.length; data++) {
            ...
            if (parserData === imgTagAlias) {
                parserObject.triggers.push([[parserData.indexOf(parserData), imgTagAlias], [parserData.indexOf(parserData), imgTagAlias]]);
                continue;
            }
    ...
    function processMarkup(markupData) {
        ...
        // Do the actual iterations to create the parserObjects and altTexts
        while (line < lines) {
            ...
            if (hasFormat) {
                ...
                for (var image = 0; image < parserObject.imageTagStore.length; image++) {
                    activeLine = activeLine.replace(parserObject.imageTagStore[image], ' <_img_> ');
        ...
        preprocessImages(cleanedData);
    }
    
    function preprocessImages(cleanedData) {
        var line = 0;
        var lines = cleanedData.length;
        var activeLine = '';
        ...
        while (line < lines) {
            ...
            var imageData = parserObject.imageData;
            ...
            words = activeLine.trim().split(" ");
            wordCount = words.length;
            
            while (currentWord < wordCount) {
                word = words[currentWord];
                
                // Get into the image display handling else process a word
                if (word.trim() === imgTagAlias) {
                    currentImage = imageStore[imgIndex];
                    ...
                    img.lineIndex = line;
                    img.wordIndex = currentWord;
                    img.imgTag = currentImage;
                    img.src = currentImage.src;
                    
                    img.spanEntirely = false;
                    img.calculatedWidth = img.width + img.borderwidth;
                    img.calculatedHeight = 0;
                                       
                    if (img.align === "center") {
                        img.spanEntirely = true;    
                    } else {
                    ...
                    if (currentImage.description !== '') {
                        ...
                        if (ci.measureText(currentImage.description).width > img.width) {
                            ...
                            descriptionLineBrake = [currentImage.description];
                        }

                        img.description = ["bottom", img.width, currentHeight];
                    }
                    
                    parserObject.imageData.push(img);
                    imgIndex++;
                }
    ...
    function drawLayout(cleanedData) {
        var line = 0;
        var lines = cleanedData.length;
        var stepY = 0;
        var stepX = 0;
        ...
        while (line < lines) {
            ...
            while (currentWord < wordCount) {
                ...
                if (hadPhantom) {
                    ...
                    if (phantomIndex >= phantomData.length) {
                        break;
                    }
                ...
                if (useFormat) {
                    if (orders[orderLevel] < 1) {
                        ...
                        var triggerWord = word.match(triggerWordMatch)[0].trim();
                        if (orders[orderLevel] > 0) {
                            setDefaultStyle();
                            resetStyle(orders[orderLevel], openTags.length);
                        }
                ...
                if (useFormat) {
                    if (triggers[orderLevel] !== false) {
                        ...
                        if (imgIndex === parserObject.imageStore.length) {
                            imgIndex++;
                            continue;
                        }
